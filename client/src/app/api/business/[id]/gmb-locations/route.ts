        ...
        const locationPromises = accountsData.accounts.map(async (account: any) => {
            try {
                const locRes = await fetch(
                    `https://mybusinessbusinessinformation.googleapis.com/v1/${account.name}/locations?readMask=name,title,storeCode,metadata,profile,categories,phoneNumbers,storefrontAddress&pageSize=100`,
                    { headers: { Authorization: `Bearer ${accessToken}` } }
                );
